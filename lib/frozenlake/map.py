...
MAPS_DIR = os.path.join(PROJECT_ROOT, "maps", "FrozenLake-v1", "test")
ACTIONS = {
    "up": 3,
    "down": 1,
    "left": 0,
...
class FrozenLakeMap(Map):
    ...
    def serialize(self, maps_dir=MAPS_DIR) -> None:
...
class FrozenLakeMapGenerator(MapGenerator):
    ...
    @classmethod
    def generate_batch(cls, map_sizes_classes: list[int], proportions: list[float], total_count: int,
                       maps_dir=MAPS_DIR) -> None:
        """
        ...
        :param maps_dir: Path to the maps
        :return:
        """
        ...
        for _id, map_size in enumerate(map_sizes):
            cls.generate_single(_id, map_size, "ansi", maps_dir)

    @classmethod
    def generate_single(cls, _id: int, size: int, render_mode: str = "ansi", maps_dir=MAPS_DIR) -> None:
        """
        Generates a single map and saves it in the project root directory.

        :param maps_dir: Path to the map
        ...
        map_obj.serialize(maps_dir=maps_dir)
...
class FrozenLakeMapSolver(MapSolver):
    ...
    @classmethod
    def solve_batch(cls, maps_dir=MAPS_DIR) -> None:
        """
        ...
        for map_file in map_files:
            cls.solve_single(map_file, maps_dir)

    @classmethod
    def solve_single(cls, map_file, maps_dir=MAPS_DIR) -> None:
        """
