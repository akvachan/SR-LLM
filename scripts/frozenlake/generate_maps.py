...
def main(map_sizes, proportions, map_count, maps_dir):
    if maps_dir:
        FrozenLakeMapGenerator.generate_batch(map_sizes, proportions, map_count, maps_dir)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--maps_dir", type=str, default=None, help="Path to the maps directory")
    args = parser.parse_args()

    if not np.isclose(sum(args.proportions), 1):
        raise ValueError("Proportions must sum to 1")

    main(args.map_sizes, args.proportions, args.map_count, args.maps_dir)
